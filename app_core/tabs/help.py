        ...
        children=[
            html.Div([
                ...
                _section('快速上手流程', [
                    ...
                    _step(4, '构成分析',
                          '打开【器类验证】确认聚类与人工标注的吻合度（ARI/NMI 指数）；'
                          '打开【器部分布】，用熵值判断每个簇捕获的是"器形特征"还是"纹饰/胎土特征"。'),
                    _step(5, '地层流动分析',
                          '打开【地层流动】，查看各地层中的簇分布。'
                          'Sankey 图揭示哪些器物群贯穿多个地层（长期器型），哪些集中于单层（单次堆积）。'
                          '打开【跨层趋势】可直接看出兴起型/衰落型/瞬间型的器物群折线。'),
                    _step(6, '网络与共现',
                          '打开【簇距离网络】查看簇的层次结构，相近的簇可能是"近亲"器物群。'
                    ...
                    html.Table([
                        ...
                        html.Tbody([
                            ...
                            _tab_row('边界样本',
                                     '找出聚类边界上的模糊样本',
                                     '调整显示数量；点击图片加入比较面板',
                                     '边界样本是距离多个簇质心都较近的陶片，可能是过渡器型或混合特征的残片'),
                            _tab_row('器类验证',
                                     '用 type_C × 簇 混淆矩阵评估聚类与人工分类的吻合度',
                                     '切换归一化方式；调整 Top-N 器类数',
                                     'ARI > 0.4 说明算法与考古分类高度一致；按器类归一化可看出哪些器类被拆散'),
                            _tab_row('器部分布',
                                     '分析每个簇内口沿/腹部/底部的构成比例',
                                     '切换归一化方式；调整最少样本数过滤小簇',
                                     '熵值低=簇以单一器部为主（捕获器形特征）；熵值高=器部混合（可能捕获纹饰/胎土特征）'),
                            _tab_row('地层流动',
                                     '分析器物群在地层序列中的分布规律',
                            ...
                            _tab_row('跨层趋势',
                                     '对每个簇画跨层占比折线图，附趋势线分类',
                                     '勾选显示类型（兴起/衰落/瞬间/稳定）；设置最少出现层数',
                                     '兴起型斜率>0.5%/层；衰落型斜率<−0.5%/层；仅出现≤2层为瞬间型'),
                            _tab_row('簇距离网络',
                                     '以质心间距离构建 kNN 网络图，揭示簇的层次结构',
                                     '调整每节点近邻 K；切换布局（PCA/UMAP）和节点颜色',
                                     '视觉上聚集的簇在特征空间中相近，可考虑合并或视为"近亲"器物群'),
                            _tab_row('共现分析',
                                     '分析哪些器物群在同一地层中共同出现',
                ...
                # ── 新分析模块详解 ─────────────────────────────────────────────
                html.Div([
                    html.H2('新分析模块详解', style=_H2),
                    html.Div([

                        html.Div([
                            html.H3('器部分布（part_C × 簇）', style=_H3),
                            html.P(
                                '热力图行 = 器部（口沿/腹部/底部等），列 = 簇编号。'
                                '按簇归一化后，颜色深浅反映该簇内各器部的占比。',
                                style=_P),
                            html.P('Shannon 熵的含义：', style={**_P, 'fontWeight': '600', 'marginBottom': '2px'}),
                            html.Ul([
                                html.Li('熵值低（接近 0）→ 单一器部主导 → 该簇捕获的是器形/廓面特征', style=_P),
                                html.Li('熵值高（接近 log₂N）→ 各器部均匀分布 → 该簇可能捕获纹饰或胎土质地特征', style=_P),
                                html.Li('语义标签"单一器部/以X为主/多器部混合"自动由主导占比和熵值计算', style=_P),
                            ], style={'paddingLeft': '18px', 'marginTop': '4px'}),
                        ], style={'flex': '1', 'minWidth': '260px', 'paddingRight': '20px'}),

                        html.Div([
                            html.H3('跨层趋势（折线图 + 趋势线）', style=_H3),
                            html.P(
                                'X 轴为地层时序（左=最新/浅，右=最早/深），Y 轴为该层内该簇的占比。'
                                '虚线为线性趋势拟合。',
                                style=_P),
                            html.Table([
                                html.Thead(html.Tr([
                                    html.Th('类型', style=_TH),
                                    html.Th('判断标准', style=_TH),
                                    html.Th('考古含义', style=_TH),
                                ])),
                                html.Tbody([
                                    html.Tr([html.Td('兴起型 ↑', style=_TD),
                                             html.Td('斜率 > +0.5%/层', style=_TD),
                                             html.Td('晚期新出现或越来越流行的器型', style=_TD)]),
                                    html.Tr([html.Td('衰落型 ↓', style=_TD),
                                             html.Td('斜率 < −0.5%/层', style=_TD),
                                             html.Td('早期流行、后期消失的器型', style=_TD)]),
                                    html.Tr([html.Td('瞬间型 ⚡', style=_TD),
                                             html.Td('出现层数 ≤ 2', style=_TD),
                                             html.Td('某一时期特有的器型或单次堆积', style=_TD)]),
                                    html.Tr([html.Td('稳定型 ─', style=_TD),
                                             html.Td('斜率接近 0', style=_TD),
                                             html.Td('长期延续、始终稳定使用的器型', style=_TD)]),
                                ]),
                            ], style={'width': '100%', 'borderCollapse': 'collapse'}),
                        ], style={'flex': '1', 'minWidth': '260px'}),

                    ], style={'display': 'flex', 'gap': '0', 'flexWrap': 'wrap', 'marginBottom': '16px'}),

                    html.Div([
                        html.H3('簇距离网络图', style=_H3),
                        html.P(
                            '节点 = 簇，大小正比于样本数；边 = kNN 连接，粗细反映质心相似度。'
                            '节点位置由质心 PCA/UMAP 2D 投影决定，与散点图的个体嵌入不同。',
                            style=_P),
                        html.P('使用建议：', style={**_P, 'fontWeight': '600', 'marginBottom': '2px'}),
                        html.Ul([
                            html.Li('PCA 布局速度快（毫秒级），UMAP 布局聚集效果更好但较慢', style=_P),
                            html.Li('余弦距离关注特征方向（推荐），欧氏距离关注特征幅值', style=_P),
                            html.Li('k=3 时边不多，便于识别孤立簇；k=5～10 时可看出更完整的层次结构', style=_P),
                            html.Li('节点颜色选"主导器类"时，同色节点聚集说明同类器物在特征空间中形成自然簇群', style=_P),
                            html.Li('底部表格展示全局最相似的 Top-10 簇对，可结合代表样本验证是否可合并', style=_P),
                        ], style={'paddingLeft': '18px', 'marginTop': '4px'}),
                    ]),
