...
function Start-Services {
    ...
    if (-not (Test-Path $PYTHON)) {
        ...
        Write-Status "请先运行: python -m venv venv && .\venv\Scripts\pip install -r requirements.txt" Yellow
        exit 1
    }
    ...
    if (Test-Path $FRPC_EXE) {
        if (Get-FrpcProcess) {
            Write-Status "frpc 已在运行，跳过启动" Yellow
        } else {
            ...
            if (Get-FrpcProcess) {
                ...
                Write-Status "✗ frpc 启动失败，请检查 $FRPC_CONF" Red
            }
        }
    } else {
    ...
    # 启动应用（若未运行）
    if (Get-AppProcess) {
        Write-Status "应用已在运行，跳过启动" Yellow
    } else {
        # 清空上次日志
        '' | Set-Content $APP_LOG
        '' | Set-Content $APP_ERR
        ...
        if ($started) {
            ...
            Write-Status "✗ 应用启动超时，请检查日志: $APP_ERR" Red
            Get-Content $APP_ERR -Tail 10 | ForEach-Object { Write-Host "  $_" -ForegroundColor DarkRed }
            exit 1
        }
    }
...
function Stop-Services {
    ...
    if ($frpc) {
        $confirm = Read-Host "是否同时停止 frpc 隧道？[y/N]"
...
function Show-Status {
    ...
    if ($appProcs) {
        ...
        $mem  = [math]::Round((Get-Process -Id $pid_).WorkingSet64 / 1MB, 1)
    ...
    if ($appProcs) {
        Write-Host ""
        Write-Status "  日志文件: $APP_LOG" Gray
        Write-Status "  错误日志: $APP_ERR" Gray
        $lastLine = Get-Content $APP_LOG -Tail 1 -ErrorAction SilentlyContinue
        if ($lastLine) { Write-Status "  最新日志: $lastLine" Gray }
    }
